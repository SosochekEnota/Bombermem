...
class Bomb(pygame.sprite.Sprite):
    ...
    def explode_bomb(self):
        ...
        for k in range(-1 * self.player.bomb_power, self.player.bomb_power + 1):
            for m in range(-1 * self.player.bomb_power, self.player.bomb_power + 1):
                if k == 0 and m > 0 and self.down_free is True:
                    if self.create_explosion(k, m):
                        self.down_free = False
                elif k == 0 and m < 0 and self.up_free is True:
                    if self.create_explosion(k, m):
                        self.down_free = False
...
class Bomb_explosion(pygame.sprite.Sprite):
    ...
    def intersection(self):
        if pygame.sprite.spritecollide(self, tiles_box_group, False):
            return True
        return False
